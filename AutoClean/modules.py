...
class FieldAssignment:
    def handle(self, df: pd.DataFrame):
        ...
        cols_num = list(df.select_dtypes(include=np.number).columns)
        cols_categ = list(
            set(df.columns) - set(df.select_dtypes(include=[np.number, "bool"]).columns)
        )
        cols_flag = list(df.select_dtypes(include=["bool"]).columns)
    ...
    def convert_to_bool(column):
        ...
        if len(unique_values) != 2:
            raise ValueError(
                "La colonne doit contenir exactement deux valeurs distinctes."
            )
    ...
    def _transform_cols_to_bool(
        self, df: pd.DataFrame, cols_categ, boolean_threshold=0.9
    ):
    ...
    def _transform_cols_to_numeric(
        ...
        for col in cols_categ:
            ...
            potential_numeric_values_percentage = (
                numeric_values / total_non_null
            ) * 100
            logger.debug(
                'Potential numeric values percentage for feature "{}": {}.',
                col,
                potential_numeric_values_percentage,
            )
            if potential_numeric_values_percentage > limit_percentage:
                logger.debug(
                    'Since potential numeric values percentage for feature "{}" is higher than the limit of {}%, the conversion to numeric will be attempted.',
                    col,
                    limit_percentage,
                )
                try:
                    ...
                    if nan_percentage > 0.5:
                        ...
                        logger.debug(
                            'Conversion to numeric failed for feature "{}", because of too many missing or invalid values. The percentage of missing values: {}',
                            col,
                            nan_percentage,
                        )
                    else:
                    ...
                    logger.debug(
                        'Conversion to numeric succeeded for feature "{}"', col
                    )
                except Exception as e:
                    logger.debug(
                        'Conversion to numeric failed for feature "{}". The error message: {}',
                        col,
                        str(e),
                    )
                    pass
